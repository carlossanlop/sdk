...
public sealed class DiffGenerator
{
    ...
    private readonly Dictionary<string, ReportDiagnostic>? _diagnosticOptions;
    ...
    /// <param name="beforeLoader">The before assembly symbol loader.</param>
    /// <param name="afterLoader">The after assembly symbol loader.</param>
    /// <param name="symbolFilter">The filter used to skip types.</param>
    /// <param name="attributeSymbolFilter">The filter used to skip attributes.</param>
    /// <param name="header">The header to use for the diff output text.</param>
    /// <param name="exceptionMessage">The default message to use for exceptions.</param>
    /// <param name="includeAssemblyAttributes">Whether to include assembly attributes in the diff output.</param>
    /// <param name="addPartialModifier">Determines whether to insert the partial modifier for types or not.</param>
    /// <param name="hideImplicitDefaultConstructors">Determines whether to hide the implicit default constructors or not.</param>
    /// <param name="diagnosticOptions">The optional diagnostic options to use when loading assemblies.</param>
    /// <param name="metadataReferences">The optional metadata references to use when loading assemblies.</param>
    /// <exception cref="DirectoryNotFoundException">The specified output directory path does not exist.</exception>
    public DiffGenerator(ILog logger,
                         ...
                         IEnumerable<MetadataReference>? metadataReferences = null)
    {
        ...
        _attributeSymbolFilter = attributeSymbolFilter;
    ...
    /// <summary>
    /// The basic diagnostic options that can be optionally used when loading assemblies.
    /// </summary>
    public static readonly Dictionary<string, ReportDiagnostic>? BasicDiagnosticOptions = new()
    {
        { "CS8019", ReportDiagnostic.Suppress }, // CS8019: Unnecessary using directive.
        { "CS8597", ReportDiagnostic.Suppress }  // CS8597: Thrown value may be null.
    };

    /// <summary>
    /// Writes the differences between the before and after configurations to the output directory.
    /// </summary>
    /// <param name="beforeAssemblySymbols">The assembly symbols from the before configuration.</param>
    /// <param name="afterAssemblySymbols">The assembly symbols from the after configuration.</param>
    /// <returns>A dictionary of assembly names and their corresponding diff output string.</returns>
    ...
    {
        CSharpAssemblyDocumentGenerator docGenerator = new(_logger,
                                                             loader,
                                                             _symbolFilter,
                                                             _attributeSymbolFilter,
                                                             _exceptionMessage,
                                                             _includeAssemblyAttributes,
                                                             _metadataReferences ?? loader.MetadataReferences,
                                                             _diagnosticOptions,
                                                             _addPartialModifier,
